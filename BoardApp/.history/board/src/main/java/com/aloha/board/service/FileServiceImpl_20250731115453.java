...
public class FileServiceImpl implements FileService {
    ...
    public boolean download(String id, HttpServletResponse response) throws Exception {
        ...
        // 파일 입력
        String fileName = file.getOriginName();     // 파일명 (다운로드시-원본파일명으로)
    ...
    public int deleteFiles(String noList) {
        ...
        for (String noStr : nos) {
            Long no = Integer.parseInt(noStr);
            delete(no);
        }

        // DB 삭제
        return fileMapper.deleteFiles(noList);
    }

    @Override
    public int deleteFilesById(String idList) {
        if(idList == null || noList.isEmpty()) return 0;
