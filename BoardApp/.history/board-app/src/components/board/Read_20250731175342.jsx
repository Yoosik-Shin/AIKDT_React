...
const Read = ({board, fileList, onDawnload}) => {


  // const fileList = [
  //   {id: 'id1', originName: '파일명1', type: 'MAIN', fileSize: '2048'},
  //   {id: 'id2', originName: '파일명2', type: 'MAIN', fileSize: '4048'},
  //   {id: 'id3', originName: '파일명3', type: 'MAIN', fileSize: '10048'}
  // ]


  const {id} = useParams()

  // board가 undefined면 로딩 메시지 또는 null 반환
  if (!board) {
    return <div>로딩 중...</div>
  }
          ...
          <td colSpan={2}>
              {
                (fileList ?? []).map((file) => (
                  <div className='flex-box' key={file.id}>
                    ...
                    <div className="item">
                      <button className='btn' onClick={() => onDawnload(file.id, file.originName)}>다운로드</button>
                    </div>
                  </div>
                ))
              }
